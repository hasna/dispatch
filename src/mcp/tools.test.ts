...
describe("MCP tool handlers", () => {
  ...
  test("targets enumerates Mosaic panes via the injected runner", async () => {
    ...
    const targets = (await tool("dispatch_targets").handler(d, { backend: "mosaic" })) as Array<{ target: string; backend: string }>;

    expect(targets).toHaveLength(1);
    expect(targets[0]).toMatchObject({ backend: "mosaic", target: "work:terminal_1", window: "Agent" });
  });
});
