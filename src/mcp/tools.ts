...
export const TOOLS: ToolDef[] = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      queue: z.boolean().optional().describe("allow queued Tab recovery recommendation for active queue-capable agents"),
    },
    handler: (deps, a) =>
      deps.client.triage({
        target: a.target as string,
  ...
  },
  {
    ...
    inputSchema: {
      ...
      includeExcerpt: z.boolean().optional().describe("include the bounded excerpt (default true)"),
      artifactPath: z.string().optional().describe("relative path under the dispatch artifacts directory for the full bounded redacted capture"),
      queue: z.boolean().optional().describe("allow queued Tab recovery for active queue-capable agents"),
